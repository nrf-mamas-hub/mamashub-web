...
import inactivatedPolioVaccineFields from "../lib/forms/inactivatedPolioVaccine"
...
export default function InactivatedPolioVaccine({userData}) {
    ...
    let [inactivatedPolioVaccinationEncounters, setInactivatedPolioVaccinationEncounters] = useState([]);
    ...
    const fieldValues = Object.values(getSections(inactivatedPolioVaccineFields, inactivatedPolioVaccinationEncounters.length, inactivatedPolioVaccinationEncounters.length+1)).flat();
    ...
    useEffect(() => {
            ...
            setVisit(visit);
            getInactivatedPolioVaccinationEncounters(visit.id);
        }, []);
    ...
    let getInactivatedPolioVaccinationEncounters = async (patientId) => {
        setLoading(true);
        ...
        setInactivatedPolioVaccinationEncounters(encounters.encounters);
        setLoading(false);
        return;
    };

    let saveInactivatedPolioVaccination = async (values) => {
        if (!visit) {
            prompt(
                "No patient visit has been initiated. To start a visit, select a patient from the Patients list."
            );
        ...
        try {
            ...
            if (immunization.status !== "success") {
                prompt("Could not submit inactivated polio vaccination details");
                return;
            }

            const inactivatedPolioObservations = {
            ...
            if (res.status === "success") {
                prompt("Inactivated Polio vaccination saved successfully");
                navigate(`/patients/${patient}`);
                await getInactivatedPolioVaccinationEncounters(patient);
                setNewVisit(false);
                return;
            } else {
        ...
        <Container sx={{ border: "1px white dashed" }}>
          ...
          {preview ? (
            <Preview
              ...
              format={getSections(
                inactivatedPolioVaccineFields,
                inactivatedPolioVaccinationEncounters.length,
                inactivatedPolioVaccinationEncounters.length + 1
              )}
              data={{ ...inputData }}
              close={() => setPreview(false)}
              submit={saveInactivatedPolioVaccination}
            />
          ) : (
                ...
                <TabPanel value="1">
                  {!newVisit && (
                    <Grid container spacing={1} padding=".5em">
                      {inactivatedPolioVaccinationEncounters.length > 0 &&
                        inactivatedPolioVaccinationEncounters.map((x, index) => {
                          return (
                  ...
                  {inactivatedPolioVaccinationEncounters.length < 1 && loading && (
                    <>
                      <CircularProgress />
                    </>
                  )}
                  ...
                  {newVisit && (
                    <>
                      <FormFields
                        ...
                        formik={formik}
                        encounters={inactivatedPolioVaccinationEncounters}
