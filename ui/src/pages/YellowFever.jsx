...
import yellowFeverVaccinationFields from "../lib/forms/yellowFever";
import { getSections } from "../lib/getFormSections";

export default function YellowFeverVaccination({userData}) {
    ...
    let [yellowFeverVaccinationEncounters, setYellowFeverVaccinationEncounters] = useState([]);
    ...
    const fieldValues = Object.values(getSections(yellowFeverVaccinationFields, yellowFeverVaccinationEncounters.length, yellowFeverVaccinationEncounters.length+1)).flat();
    ...
    useEffect(() => {
            ...
            setVisit(visit);
            getYellowFeverVaccinationEncounters(visit.id);
        }, []);
    ...
    let getYellowFeverVaccinationEncounters = async (patientId) => {
        setLoading(true);
        let encounters = await (
            await FhirApi({
                url: `/crud/encounters?patient=${patientId}&encounterCode=${"YELLOW_FEVER_VACCINATION"}`,
            })
        ).data;
        setYellowFeverVaccinationEncounters(encounters.encounters);
        setLoading(false);
        return;
    };
    ...
    let saveYellowFeverVaccination = async (values) => {
        ...
        try {
            ...
            if (res.status === "success") {
                ...
                await getYellowFeverVaccinationEncounters(patient);
                setNewVisit(false);
                return;
            } else {
    ...
    <>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <Container sx={{ border: "1px white dashed" }}>
          ...
          {preview ? (
            <Preview
              title="Yellow Fever Vaccine Preview"
              format={getSections(
                yellowFeverVaccinationFields,
                yellowFeverVaccinationEncounters.length,
                yellowFeverVaccinationEncounters.length + 1
              )}
              data={{ ...inputData }}
              close={() => setPreview(false)}
              submit={saveYellowFeverVaccination}
            />
          ) : (
            <form onSubmit={formik.handleSubmit}>
              <TabContext value={value}>
                <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
                  ...
                  >
                    <Tab label="Yellow Fever Vaccine" value="1" />
                  </TabList>
                </Box>
                <TabPanel value="1">
                  {!newVisit && (
                    <Grid container spacing={1} padding=".5em">
                      {yellowFeverVaccinationEncounters.length > 0 &&
                        yellowFeverVaccinationEncounters.map((x, index) => {
                          return (
                  ...
                  {yellowFeverVaccinationEncounters.length < 1 && loading && (
                    <>
                      <CircularProgress />
                    </>
                  )}
                  ...
                  {newVisit && (
                    <>
                      <FormFields
                        ...
                        formik={formik}
                        encounters={yellowFeverVaccinationEncounters}
