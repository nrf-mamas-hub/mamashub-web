import {
  ...
  import micronutrientPowdersFields from "../lib/forms/micronutrientPowders";

export default function MotherPostnatalCare({ userData }) {
    ...
    let [micronutrientPowders, setmicronutrientPowders] = useState({});
    let [micronutrientPowdersEncounters, setmicronutrientPowdersEncounters] = useState(
        []
    );
    ...
    const fieldValues = Object.values(micronutrientPowdersFields).flat();
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        ...
        if (visit) {
            getMicronutrientPowdersEncounters(visit.id);
        }
      }, []);
      ...
      let getMicronutrientPowdersEncounters = async (patientId) => {
        setLoading(true);
        let encounters = await (
          await FhirApi({
            url: `/crud/encounters?patient=${patientId}&encounterCode=${"MICRONUTRIENT_POWDERS"}`,
          })
        ).data;
        // console.log(encounters);
        setmicronutrientPowdersEncounters(encounters.encounters);
        setLoading(false);
        return;
      };
      let saveMicronutrientPowdersCare = async (values) => {
        //get current patient
        if (!visit) {
          prompt(
        ...
        try {
          //create Encounter
          let encounter = await createEncounter(patient, "MICRONUTRIENT_POWDERS");
        ...
        <>
          <LocalizationProvider dateAdapter={AdapterDateFns}>
            <Container sx={{ border: "1px white dashed" }}>
              ...
              {preview ? (
                <Preview
                  title="Micronutrient powders preview"
                  format={micronutrientPowdersFields}
                  data={{ ...inputData }}
                  close={() => setPreview(false)}
                  submit={saveMicronutrientPowdersCare}
                />
              ) : (
                <form onSubmit={formik.handleSubmit}>
                  <TabContext value={value}>
                    <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
                      ...
                      >
                        <Tab label="Micronutrient Powders (MNPs)" value="1" />
                      </TabList>
                    </Box>
                    <TabPanel value="1">
                      {/* <p></p> */}
                      {!newVisit && (
                        <Grid container spacing={1} padding=".5em">
                          {micronutrientPowdersEncounters.length > 0 &&
                            micronutrientPowdersEncounters.map((x, index) => {
                              return (
                          ...
                          {micronutrientPowdersEncounters.length < 17 && (
                            <Grid
                              item
                              xs={12}
                              md={12}
                      ...
                      {micronutrientPowdersEncounters.length < 1 && loading && (
                        <>
                          <CircularProgress />
                        </>
                      )}
                      ...
                      {newVisit && (
                        <>
                          <FormFields
                            formData={micronutrientPowdersFields}
                            formik={formik}
                            encounters={micronutrientPowdersEncounters}
