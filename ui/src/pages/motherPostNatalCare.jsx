...
export default function MotherPostnatalCare({ userData }) {
    ...
    const fieldValues = Object.values(postnatalCareFields).flat();
    ...
    useEffect(() => {
      ...
      let getMotherPostNatalCareEncounters = async (patientId) => {
        setLoading(true);
        ...
        setmotherPostNatalCareEncounters(encounters.encounters);
        setLoading(false);
        return;
      };
      let saveMotherPostNatalCare = async (values) => {
        //get current patient
        if (!visit) {
          prompt(
        ...
        try {
          //create Encounter
          let encounter = await createEncounter(patient, "MOTHER_POSTNATAL_CARE");
          ...
          if (res.status === "success") {
            ...
            await getMotherPostNatalCareEncounters(patient);
            setNewVisit(false);
            return;
          } else {
            prompt(res.error);
